...
- **Conversas em abas (read-only).** A caixa tem aba `compartilhado` (mensagens entre agentes) + uma aba por agente com o **stream próprio do transcript** (o que ele diz/envia/recebe), via `GET /api/agent/<nome>`, últimas 50. As abas saem de `config.json` (team vivo) ou, se o team já foi limpo, dos agentes detectados nos transcripts (`streamAgents`). Identidade vem do briefing ``no time `<team>` ``; o orchestrator (manda pros membros) é o team-lead. Não há envio — só leitura.
- **Chat completo por agente (overlay).** Clicar no card do agente (ou em "abrir chat completo" na aba dele) abre um overlay que imita o chat do Claude: pensamento (`thinking`), texto, execuções de ferramenta (nome + comando + resultado expansível), enviadas/recebidas — via `GET /api/agent/<nome>/full`, últimas 160, tail vivo. Filtra ruído (erros de rate-limit, `idle_notification`). Thinking só aparece se o transcript o reteve — runs antigos podem ter só a assinatura (vazio); team vivo com extended-thinking mostra.
- **`.claude/launch.json` é local/efêmero.** Config do `preview_start` (caminho fixo da ferramenta), com TEAM/REPO/PORT hardcoded; muda a cada team e morre com ele. Está no `.gitignore` — nunca commitar nem versionar.
- **Chat por-agente vs teammates in-process.** Teammates `in-process` (o default) NÃO têm transcript próprio — o tráfego vive no transcript da sessão-lead. Por isso: o conjunto de membros vem do `config.json` (fonte viva, não dos briefings), o chat do **team-lead** carrega toda a conversa inter-agente, e o chat de cada **membro** é reconstruído do transcript do lead (mensagens trocadas; o raciocínio/tools internos do membro só aparecem se ele for pane/tmux-backed com transcript dedicado). Identidade de briefing aceita "no time" e "on/in/of team".
