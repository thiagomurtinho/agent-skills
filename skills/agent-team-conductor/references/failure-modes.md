...
| Tried to set up a foreman / intermediate "team-lead" teammate / 3-level hierarchy | The structure is **exactly 2 levels**: the chat is the one lead (the only spawner); teammates are flat and **can't spawn**. A teammate-as-foreman can't actually create workers. | Flatten to 2 levels: the chat (lead) spawns and coordinates all subordinates directly. Don't designate a teammate as a sub-lead — it has no one to lead. |
